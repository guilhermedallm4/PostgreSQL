...
--DELETE ITEM CESTA
CREATE FUNCTION T_DELET_CESTA(IN ID_CESTA INT)
RETURNS VARCHAR
AS $$
DECLARE
	QNT INTEGER;
...
BEGIN
	SELECT QNT_ITEM FROM CESTA
	WHERE IDCESTA = ID_CESTA INTO QNT;
	
	SELECT ID_PRODUTO FROM CESTA
	WHERE ID_CESTA = ID_CESTA INTO ID_PRODUTO_CESTA;
	
    DELETE FROM CESTA WHERE IDCESTA = ID_CESTA;

	UPDATE PRODUTO SET QUANTIDADE = QUANTIDADE + QNT
	WHERE IDPRODUTO = ID_PRODUTO_CESTA;
