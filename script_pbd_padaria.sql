...
CREATE FUNCTION DELETE_FUN(IN CPF_FUN VARCHAR(14))
RETURNS VARCHAR
AS $$
DECLARE
	NOME_FUN VARCHAR(30);
...
--DELETE ITEM CESTA
CREATE FUNCTION T_DELETCESTA(IN ID_CESTA INTEGER)
RETURNS VARCHAR
AS $$
DECLARE
	QNT INTEGER;
...
CREATE FUNCTION ADDCLIENTE(IN NAME_CLI varchar(30),IN TELE_CLI varchar(30), IN EMAIL_CLI VARCHAR(30), IN CPF_CLI VARCHAR(30))
...
CREATE FUNCTION ADDFUNCIONARIO(IN NAME_FUN varchar(30),IN TELE_FUN varchar(30), IN EMAIL_FUN VARCHAR(30), IN CPF_FUN VARCHAR(30),
...
BEGIN
	...
	ELSE
		...
		ELSE
			...
			RETURN QUERY SELECT P.NOME, C.QNT_ITEM, P.VALOR FROM PRODUTO AS P
			INNER JOIN CESTA AS C
			ON P.IDPRODUTO = C.ID_PRODUTO
...
CREATE FUNCTION ADDFORNECEDOR(IN NOME_FOR VARCHAR(30), IN TELEFONE_FOR VARCHAR(11), IN ENDERECO_FOR VARCHAR(30) ,IN EMAIL_FOR VARCHAR(30), IN CNPJ_FOR VARCHAR(14))
...
--ADICIONAR PRODUTOS
CREATE FUNCTION ADD_PRODUCT(IN NOME_PRODUTO VARCHAR(30), VALOR_PRODUTO VARCHAR(30), VALIDADE_PRODUTO VARCHAR(30), TIPO_PRODUTO VARCHAR(30), QUANTIDADE_PRODUTO VARCHAR(30))
RETURNS TABLE (NOME VARCHAR, VALOR FLOAT, VALIDADE DATE, TIPO VARCHAR, QUANTIDADE INT)
AS $$
DECLARE COMPARATION_ID
BEGIN
...
BEGIN
	...
	ELSE
		...
		SELECT NOME FROM PRODUTO WHERE NOME = NOME_PRODUTO INTO NAME_PRODUCT;
		DELETE FROM PRODUTO WHERE NOME = NOME_PRODUTO;
	END IF;
	RETURNS NAME_PRODUCT;
END;
$$ LANGUAGE PLPGSQL;
